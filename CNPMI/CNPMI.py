...
def read_yaml_cnpmi(path):
    with open(path) as file:
        config = yaml.safe_load(file)
    return config


def read_texts_cnpmi(path):
    texts = list()
...
def split_text_word_cnpmi(texts):
...
def make_bow(corpus_path, vocab):
    corpus = read_texts_cnpmi(corpus_path)
...
def main():
    ...
    parallel_corpus_tuples = read_yaml_cnpmi(args.ref_corpus_config)['parallel_corpus_tuples']

    sep_token = '|'

    topics1 = read_texts_cnpmi(args.topics1)
    topics1 = split_text_word_cnpmi(topics1)
    topics2 = read_texts_cnpmi(args.topics2)
